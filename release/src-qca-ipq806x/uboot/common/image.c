...
#if defined(ASUS_PRODUCT) && !defined(USE_HOSTCC)
static char* get_addr_name(ulong addr)
{
	...
	for (i = 0, desc = &g_s2_loc.desc[0]; i < g_s2_loc.count; ++i, ++desc) {
		if ((unsigned char*)addr != desc->code)
			continue;
...
void genimg_get_dtb_from_asus_trx(bootm_headers_t *images)
{
	int i;
	image_header_t *hdr;
	version_t *hw;
	...
	if (!images || !images->legacy_hdr_valid || !images->legacy_hdr_os) {
		debug("%s: invalid images %p or legacy_hdr_os!\n", __func__, images);
		return;
	}
	...
	for (i = 0; i < (MAX_VER*2); i++, hw++) {
		...
		rfs_addr = images->lastAddr + rfs_offset;

		if (!rfs_offset || fdt_check_header((void*)rfs_addr))
			continue;

		debug("Found DTB at %08x\n", rfs_addr);
		if ((images->dtb_addr = (ulong)malloc(fdt_totalsize(rfs_addr))) != 0) {
			memmove((void*)images->dtb_addr, (void*)rfs_addr, fdt_totalsize(rfs_addr));
		} else {
			printf("%s: Allocate %d bytes to keep DTB failed!\n", __func__, fdt_totalsize(rfs_addr));
			images->dtb_addr = rfs_addr;
		}
...
ulong genimg_get_image(ulong img_addr)
{
	...
	if (img_addr < CONFIG_SYS_FLASH_BASE)
		return ram_addr;

	/* get RAM address */
	ram_addr = CONFIG_SYS_LOAD_ADDR;

	if (img_addr == images.img_flash_addr && ram_addr == images.lastAddr &&
	    genimg_get_format((void *)ram_addr) == IMAGE_FORMAT_LEGACY &&
	    image_get_data_size((const image_header_t *)ram_addr) == images.lastLen) {
		images.verify = 0;
		return ram_addr;
	}
	...
	ra_flash_read((uchar*) ram_addr, img_addr, h_size);
	...
	/* read in image data */
	debug("   Reading image remaining data from flash address "
		"%08lx to RAM address %08lx\n", ra_flash_offset(img_addr + h_size),
		ram_addr + h_size);

	ra_flash_read((uchar*)(ram_addr + h_size), img_addr + h_size, d_size);
	images.img_flash_addr = img_addr;
#endif	/* ASUS_PRODUCT */
		...
		uint8_t arch, ulong *rd_start, ulong *rd_end)
{
	...
	*rd_end = 0;
	...
	if ((argc >= 3) && (strcmp(argv[2], "-") ==  0)) {
		...
		uint32_t rfs_offset, rfs_addr, dtb_len;
		rfs_addr = rfs_offset = u.rfs_offset_net_endian = 0;

#if defined(CONFIG_DUAL_TRX)
		if (images->img_flash_addr >= cfg_kern2_addr) {
			images->root_id = 1;	/* linux2 */
		}
		else
#endif
		if (images->img_flash_addr >= CONFIG_SYS_FLASH_BASE) {
			images->root_id = 0;	/* linux */
		}
		else if (images->rescue_image) {
			/* Don't handle RAM disk. */
		}
		else {
			...
			for (i = 0; i < (MAX_VER*2); i++, hw++) {
				...
				if (!fdt_check_header((void*)rfs_offset)) {
					dtb_len = be32_to_cpu(*(uint32_t*)(rfs_offset + 4));
					dtb_len += 63;
					dtb_len &= ~((1UL << 6) - 1);
					printf("%s: Shift rfs_offset from %08x to %08x due to DTB!\n",
						__func__, rfs_offset, rfs_offset + dtb_len);
					rfs_offset += dtb_len;
				}

				rd_len = ntohl(hdr->ih_size) + sizeof(image_header_t) - rfs_offset;
				break;
			}
		}
...
#if defined(ASUS_PRODUCT)
	if (rd_data && rd_len) {
		*rd_start = images->os.load + (16 << 20);	/* FIXME */
		*rd_start &= ~(4096 - 1);			/* align on page */
		*rd_end = *rd_start + rd_len;
		memmove((void*)*rd_start, (void*)rd_data, rd_len);
	} else if (rd_data) {
		*rd_start = rd_data;
...
#if defined(ASUS_PRODUCT)
	if (images->dtb_addr) {
		if (fdt_magic(images->dtb_addr))
			fdt_blob = (char*) images->dtb_addr;
		else
			printf("Invalid DTB magic %08x @ %08lx\n", fdt_magic(images->dtb_addr), images->dtb_addr);
	} else
#endif
