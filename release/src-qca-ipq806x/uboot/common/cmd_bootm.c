...
int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
#else
...
#endif
{
	...
	genimg_get_dtb_from_asus_trx(&images);
...
#if defined(ASUS_PRODUCT)
	images.lastAddr = img_addr;
	images.lastLen = image_get_data_size(hdr);
#endif
...
#if defined(ASUS_PRODUCT)
	if (!img_addr)
		return NULL;	/* *os_len must be zero. */
#endif
