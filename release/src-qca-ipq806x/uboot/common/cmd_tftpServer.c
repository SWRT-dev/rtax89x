...
static int load_asus_firmware(cmd_tbl_t *cmdtp, int argc, char * const argv[])
{
	...
	/* In most cases, we use CONFIG_SYS_LOAD_ADDR to load firmware without heading image header.
	 ...
	 */
	unsigned char *buf_addr = (unsigned char*)(CONFIG_SYS_LOAD_ADDR - sizeof(image_header_t));
	...
	sprintf(addr2, "%lx", cfg_kern2_addr);
	...
	if (src && dest && len) {
		...
		if (r) {
			printf("Read firmware from %x, length %x fail. (r = %d)\n", src, len, r);
		} else {
