...
static inline uint32_t athrs17_reg_read(ipq_gmac_board_cfg_t *gmac_cfg, uint32_t reg_addr) { return 0; }
static inline void athrs17_reg_write(ipq_gmac_board_cfg_t *gmac_cfg, uint32_t reg_addr, uint32_t reg_val) { return; }
...
{
#if defined(CONFIG_SWITCH_QCA8337)
