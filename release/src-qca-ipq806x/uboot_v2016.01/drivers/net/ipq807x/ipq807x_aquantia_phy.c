...
#if defined(CONFIG_ASUS_PRODUCT)
static int loaded_aqr_fw = 0;

extern const unsigned char _binary_drivers_net_aqr_cld_gz_start[], _binary_drivers_net_aqr_cld_gz_end[];
#endif
...
/**
 * Extract gzipped AQR firmware and upload it to AQR PHY.
 * @return:
 * 	0:	success
 *     -1:	gunzip error
 *     -2:	PHY::Scratchpad Read/Write test fail
 *     -3:	CRC check failed on phy fw file
 */
int ipq_board_fw_download(unsigned int phy_addr)
{
#if defined(CONFIG_ASUS_PRODUCT)
	int ret, d_status = dcache_status();
	void *load_buf;
	unsigned long src_len, dst_len = 0;
	const unsigned char *p = _binary_drivers_net_aqr_cld_gz_start, *end = _binary_drivers_net_aqr_cld_gz_end;

	if (!aqr_phy_chip())
		return 0;
	if (!p || !end || (end - p) <= 0) {
		printf("bad ETHPHYFW blob\n");
		return -1;
	}
	if (loaded_aqr_fw) {
		printf("AQR firmware loaded\n");
		return 0;
	}
	...
	if (!d_status)
		dcache_enable();
	src_len = end - p;
	dst_len = 4 * src_len;	/* compress ratio is 1.8x. 4 x compress length should enough. */
	load_buf = malloc(dst_len);
	if (!load_buf) {
		load_buf = (void*) CONFIG_SYS_LOAD_ADDR;
	}
	ret = gunzip(load_buf, dst_len, (unsigned char *) p, &src_len);	/* 0: success; -1: error */
	if (ret < 0) {
		printf("gunzip AQR fw. failed. (ret %d, dst_len %lu, src_len %lu)\n",
			ret, dst_len, src_len);
		goto exit_1;
	}

	ret = program_ethphy_fw(phy_addr, (uint32_t) load_buf, (uint32_t) src_len);
	if (!ret)
		loaded_aqr_fw = 1;

 exit_1:
	if (load_buf != (void*) CONFIG_SYS_LOAD_ADDR)
		free(load_buf);
	if (!d_status)
		dcache_disable();
...
/**
 * Upload firmware to AQR PHY.
 * @return:
 * 	0:	success
 *     -1:	N/A, don't use it, reserved it to gunzip()
 *     -2:	PHY::Scratchpad Read/Write test fail
 *     -3:	CRC check failed on phy fw file
 */
