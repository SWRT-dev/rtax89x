...
#if defined(CONFIG_QCA8075_PHYBASE)
#define QCA8075_PHYBASE	CONFIG_QCA8075_PHYBASE
#else
#define QCA8075_PHYBASE	0
#endif
...
extern unsigned int g_ppe_rule;
...
static int ipq807x_eth_snd(struct eth_device *dev, void *packet, int length)
{
	...
	if (tftp_acl_our_port != tftp_our_port) {
		...
		ipq807x_ppe_acl_set(g_ppe_rule, 0x4, 0x1, tftp_our_port, 0xffff, 0, 0);
...
static int ipq807x_eth_init(struct eth_device *eth_dev, bd_t *this)
{
	...
	char *dp[] = {"Half", "Full"};
	char msg[sizeof("ethXXX PHYXXX up Speed :1000 Full duplexYYYYYY")];
	int linkup=0;
	int mac_speed = 2, speed_clock1 = 0x101, speed_clock2 = 0;
	int phy_addr, real_phy_addr, port_8033 = -1, node, aquantia_port = -1;
	int sfp_port = -1;
	int phy_node = -1;
	int ret_sgmii_mode;
	...
	int s;
	ushort r0 = 0, r1 = 0, r11 = 0;
	fal_port_speed_t spd[4] = { FAL_SPEED_10, FAL_SPEED_100, FAL_SPEED_1000, FAL_SPEED_BUTT };
#endif
...
next_loop:
	debug("Detect links, retry %d\n", retry);
	t2 = get_timer(0);
	if (g_rescue_mode && (t2 - t1) > (TFTPD_TIMEOUT * CONFIG_SYS_HZ)) {
		if ((g_led_intv & 1) == 0) {
			func_led_off();
		} else {
			func_led_on();
	 ...
	 */
#if defined(GTAXY16000) || defined(RTAX89U) || defined(GTAX6000S)
	/* LAN3 ~ LAN7, QCA8337, PHY 5 ~ 1 */
	for (i = 1; i <= 5; ++i) {
		data = ar8xxx_read(0x7C + 4 * i);
		status = !!(data & (1U << 8));
		...
		if (status) {
			...
			speed = spd[data & 3];
			if (speed == FAL_SPEED_BUTT) {
				printf("eth%d PHY%d %s Speed : unknown\n",
					priv->mac_unit, i - 1, lstatus[!status]);
				continue;
			}
			printf("eth%d PHY%d %s Speed :%d %s duplex\n",
				priv->mac_unit, i - 1, lstatus[!status], speed, dp[duplex]);
		}
	}

	/* LAN8, AR8033, PHY 6 */
	i = 6;
	if (ipq_mdio_read(i, 1, &r1) >= 0 &&
	    ipq_mdio_read(i, 0, &r0) >= 0 &&
	    ipq_mdio_read(i, 0x11, &r11) >= 0)
	{
		s = 0;
		status = !!(r1 & (1U << 2));
		...
		if (status) {
			...
			if (speed == FAL_SPEED_BUTT) {
				printf("eth%d PHY%d %s Speed : unknown\n",
					priv->mac_unit, i, lstatus[!status]);
			} else {
				printf("eth%d PHY%d %s Speed :%d %s duplex\n",
					priv->mac_unit, i, lstatus[!status], speed, dp[duplex]);
			}
		}
	}
#endif
	...
	for (i =  0; i < PHY_MAX; i++) {
		...
		if (i == sfp_port) {
			...
			duplex = FAL_FULL_DUPLEX;
			real_phy_addr = 31;	/* Fake PHY id for SFP+ port */
		} else {
			if (!priv->ops[i]) {
				debug("Phy%d ops not mapped\n", i);
				continue;
			}
			...
			if (phy_node >= 0) {
				real_phy_addr = phy_addr = phy_info[i]->phy_address;
			} else {

				if (i == port_8033)
					real_phy_addr = phy_addr = QCA8033_PHY_ADDR;
				else if (i == aquantia_port)
					real_phy_addr = phy_addr = AQU_PHY_ADDR;
				else {
					phy_addr = i;
					real_phy_addr = QCA8075_PHYBASE + i;
				}
			}
...
#if defined(GTAXY16000) || defined(RTAX89U) || defined(GTAX6000S)
		/* QCA8075 port0 cascades QCA8337 and then AR8033 and always has link.
		 * Don't count it's link status.
		 */
		if (status == 0 && i != 0)
			linkup++;
#else
		...
		switch (speed) {
			case FAL_SPEED_10:
				...
				speed_clock1 = 0x109;
				speed_clock2 = 0x9;
				snprintf (msg, sizeof(msg), "eth%d PHY%d %s Speed :%d %s duplex\n",
						priv->mac_unit, real_phy_addr, lstatus[status], speed,
						dp[duplex]);
				if (phy_node >= 0) {
		...
		if (i == aquantia_port)
			...
			ipq807x_pqsgmii_speed_set(i, mac_speed, 0);
...
void __ipq_qca_aquantia_phy_init(void)
{
	int node, aquantia_port = -1;

	node = fdt_path_offset(gd->fdt_blob, "/ess-switch");
	if (node >= 0)
		aquantia_port = fdtdec_get_uint(gd->fdt_blob, node, "aquantia_port", -1);
	ipq_qca_aquantia_phy_init(&ipq807x_edma_dev[0]->ops[aquantia_port], CONFIG_AQR_PHYADDR);
}
#endif
#endif
...
int ipq807x_edma_init(void *edma_board_cfg)
{
	...
	int port_8033 = -1, node, phy_addr, real_phy_addr, aquantia_port = -1;
	int mode, phy_node = -1;
	...
	for (i = 0; i < IPQ807X_EDMA_DEV; edma_cfg++, i++) {
		...
		for (phy_id =  0; phy_id < PHY_MAX; phy_id++) {
			if (phy_node >= 0) {
				real_phy_addr = phy_addr = phy_info[phy_id]->phy_address;
			} else {
				if (phy_id == port_8033)
					real_phy_addr = phy_addr = QCA8033_PHY_ADDR;
					...
					phy_addr = phy_id;
					real_phy_addr = QCA8075_PHYBASE + phy_id;
				}
			}

			phy_chip_id1 = ipq_mdio_read(real_phy_addr, QCA_PHY_ID1, NULL);
			phy_chip_id2 = ipq_mdio_read(real_phy_addr, QCA_PHY_ID2, NULL);
			...
			switch(phy_chip_id) {
				...
				case QCA8075_PHY_V1_1_2P:
					...
					if (!sw_init_done) {
#if defined(GTAXY16000) || defined(RTAX89U) || defined(GTAX6000S)
						/* Configure QCA8337 switch. */
						ar8xxx_write(0x7c, 0x0000007e);/* PORT0_STATUS */
						ar8xxx_write(0x94, 0x0000037e);/* PORT6_STATUS */
						ar8xxx_write(0x04, 0x05080000);/* PORT0_PAD_CTRL */
						ar8xxx_write(0x08, 0x01000000);/* PORT5_PAD_CTRL */
						ar8xxx_write(0x0c, 0x00000080);/* PORT6_PAD_CTRL */
						ar8xxx_write(0xe0, 0xc78164de);/* SGMII_CTRL */
						ar8xxx_write(0x624, 0x007f7f7f);/* GLOBAL_FW_CTRL1 */
						ar8xxx_write(0x54, 0x0000007e);/* LED_CTRL1 */
						printf("PORT0/5/6_PAD_CTRL 0x%08x/%08x/%08x\n",
							ar8xxx_read(0x4), ar8xxx_read(0x8), ar8xxx_read(0xc));
						printf("PORT0/5/6_STATUS   0x%08x/%08x/%08x\n",
							ar8xxx_read(0x7c), ar8xxx_read(0x90), ar8xxx_read(0x94));
#endif
				...
				case AQUANTIA_PHY_113A0:
				case AQUANTIA_PHY_113A1:
				case AQUANTIA_PHY_113B0:
#if !defined(CONFIG_ASUS_PRODUCT)
