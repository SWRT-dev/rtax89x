...
void ipq807x_ppe_interface_mode_init(void)
{
	...
	printf("switch_mac_mode: 0x%x/%x/%x\n", mode0, mode1, mode2);
...
void ipq807x_ppe_provision_init(void)
{
	...
	g_ppe_rule = 0;
	ipq807x_ppe_acl_set(g_ppe_rule++, ADPT_ACL_HPPE_IPV4_DIP_RULE, UDP_PKT, 67, 0xffff, 0, 0);
	ipq807x_ppe_acl_set(g_ppe_rule++, ADPT_ACL_HPPE_IPV4_DIP_RULE, UDP_PKT, 68, 0xffff, 0, 0);
#if defined(CONFIG_ASUS_PRODUCT)
	/* Allowing Firmware Restoration packets */
	ipq807x_ppe_acl_set(g_ppe_rule++, ADPT_ACL_HPPE_IPV4_DIP_RULE, UDP_PKT, 69, 0xffff, 0, 0);
#endif
	/* Dropping all the UDP packets */
	ipq807x_ppe_acl_set(g_ppe_rule++, ADPT_ACL_HPPE_IPV4_DIP_RULE, UDP_PKT, 0, 0, 0, 1);
}
