...
{
	if (phy_id + QCA8075_PHYBASE > 0x20)
		printf("%s: base %d phy %d exceeds max. address\n", __func__, QCA8075_PHYBASE, phy_id);
	ipq_mdio_write(QCA8075_PHYBASE + phy_id, reg_id, reg_val);
	return 0;
}
...
static u16 qca8075_phy_reg_read(u32 dev_id, u32 phy_id, u32 reg_id)
{
	if (phy_id + QCA8075_PHYBASE > 0x20)
		printf("%s: base %d phy %d exceeds max. address\n", __func__, QCA8075_PHYBASE, phy_id);
	return ipq_mdio_read(QCA8075_PHYBASE + phy_id, reg_id, NULL);
}
