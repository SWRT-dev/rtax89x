...
#define QC98XX_EEPROM2_OFFSET		0x1000
#endif
#define QC98XX_EEPROM_SIZE_LARGEST	131072
 ...
 * last NAND Flash page of every copy in Factory/Factory2 volume.
 */
#define EEPROM_SET_HEADER_OFFSET	(CFG_UBI_FACTORY_SIZE - 2*1024)
#define MAX_EEPROM_SET_LENGTH		EEPROM_SET_HEADER_OFFSET
/* two eeprom set per factory volume. two factory volume. */
#define NR_EEPROM_SETS		((CFG_UBI_FACTORY_SIZE + CFG_UBI_FACTORY2_SIZE) / CFG_UBI_FACTORY_SIZE)
...
static int upadte_wifi_eeprom_csum(void *ptr)
{
	int i;
	uint16_t *p = ptr, orig_csum, sum = 0;
#if defined(CONFIG_ARCH_IPQ807x)
	const int csum_offset = 5;
#elif defined(CONFIG_ARCH_IPQ806x)
	const int csum_offset = 1;
#else
#error Define checksum offset!
#endif

	if (!ptr)
		return -1;

	orig_csum = *(p + csum_offset);
	*(p + csum_offset) = 0;
	for (i = 0; i < (QC98XX_EEPROM_SIZE_LARGEST / 2); ++i, ++p)
		sum ^= __le16_to_cpu(*p);

	p = ptr;
	*(p + csum_offset) = __cpu_to_le16(sum ^ 0xFFFF);
 ...
 * If 2-nd copy of Factory volume is choosed, add CFG_UBI_FACTORY_SIZE to it.
 * If 1-st copy of Factory2 volume is choosed, add 2 * CFG_UBI_FACTORY_SIZE to it.
 * If 2-nd copy of Factory2 volume is choosed, add 3 * CFG_UBI_FACTORY_SIZE to it.
 */
...
const static struct ubi_vol_s {
	...
	{ "Factory",	CFG_UBI_FACTORY_SIZE,		1 },
	{ "Factory2",	CFG_UBI_FACTORY2_SIZE,		1 },
	{ "linux",	CFG_UBI_ALT_FIRMWARE_SIZE,	1 },
#if defined(CONFIG_DUAL_TRX)
	{ "linux2",	CFG_UBI_ALT_FIRMWARE2_SIZE,	1 },
#endif
	/* last volume size would be auto-resize according to free space */
	{ "jffs2",	CFG_UBI_ALT_APP_SIZE,		1 },
	{ "", 0, 0 },
#endif
};
...
char *get_ubi_volume_param(unsigned long addr, unsigned long *offset, unsigned int *size)
{
	...
	const struct ubi_vol_s *p = &ubi_vol[0];
#if defined(CONFIG_ALTERNATE_FLASH_SIZE)
	...
	nand = &nand_info[dev];
	if (nand->size == CONFIG_ALTERNATE_FLASH_SIZE)
		p = &ubi_alt_vol[0];
#endif
...
/* Resize linux volume.
 * @ubi:	pointer to struct ubi_device
 * @return:
 *	0:	success
 *    < 0:	error
 */
int resize_linux_volume(struct ubi_device *ubi, struct ubi_volume *linux_vol, const struct ubi_vol_s *linux_def)
{
	int rcode;
	unsigned int old_size = 0;
	char **p, vol_name[sizeof("linux2XXXXXX")];
	char *vol_tbl[] = { "linux", "linux2", "jffs2", NULL };
	char load_addr_str[sizeof("00000000XXX")], size_str[sizeof("00000000XXX")];
	char *ubi_read_vol[] = { "ubi", "readvol", load_addr_str, vol_name, size_str };
	char *ubi_remove_vol[] = { "ubi", "remove", vol_name };
	char *ubi_create_vol[] = { "ubi", "createvol", vol_name, size_str, "dynamic" };
	char *ubi_write_vol[] = { "ubi", "writevol", load_addr_str, vol_name, size_str };

	if (!ubi || !linux_vol || !linux_def)
		return -1;

	/* Backup original linux volume. */
	old_size = linux_vol->reserved_pebs * linux_vol->usable_leb_size;
	snprintf(load_addr_str, sizeof(load_addr_str), "%x", CONFIG_SYS_LOAD_ADDR);
	snprintf(size_str, sizeof(size_str), "%x", old_size);
	strlcpy(vol_name, linux_def->name, sizeof(vol_name));
	rcode = do_ubi(NULL, 0, ARRAY_SIZE(ubi_read_vol), ubi_read_vol);
	if (rcode < 0) {
		printf("%s: Can't read %s volume. rcode 0x%x\n", __func__, linux_def->name, rcode);
		return -2;
	}

	/* Remove linux, linux2, and jffs2 volume. */
	for (p = vol_tbl; *p != NULL; ++p) {
		strlcpy(vol_name, *p, sizeof(vol_name));
		rcode = do_ubi(NULL, 0, ARRAY_SIZE(ubi_remove_vol), ubi_remove_vol);
		if (rcode < 0) {
			printf("%s: Can't remove %s volume. rcode 0x%x\n", __func__, *p, rcode);
		}
	}

	/* Create new linux volume. */
	strlcpy(vol_name, linux_def->name, sizeof(vol_name));
	snprintf(size_str, sizeof(size_str), "%x", linux_def->size);
	rcode = do_ubi(NULL, 0, ARRAY_SIZE(ubi_create_vol), ubi_create_vol);
	if (rcode < 0) {
		printf("%s: Can't create new %s volume. rcode 0x%x\n", __func__, linux_def->name, rcode);
		return -4;
	}

	/* Restore linux volume. */
	snprintf(size_str, sizeof(size_str), "%x", old_size);
	rcode = do_ubi(NULL, 0, ARRAY_SIZE(ubi_write_vol), ubi_write_vol);
	if (rcode < 0) {
		printf("%s: Can't restore new %s volume. rcode 0x%x\n", __func__, linux_def->name, rcode);
		return -5;
	}

	printf("Resize %s volume success.\n", linux_def->name);
	return 0;
}
#endif
...
int init_ubi_volumes(struct ubi_device *ubi)
{
	...
	struct ubi_volume *v;
	unsigned int size;
	const struct ubi_vol_s *p = &ubi_vol[0], *q;
	...
	nand = &nand_info[dev];
	if (nand->size == CONFIG_ALTERNATE_FLASH_SIZE) {
		printf("Flash size %luMB, select alternate flash layout.\n", (ulong) nand->size >> 20);
		p = &ubi_alt_vol[0];
		cfg_kern2_addr = CFG_ALT_KERN2_ADDR;
	}
#endif

	for (id = 0; p->size; ++p, ++id) {
		q = p + 1;
...
#if defined(GTAXY16000) || defined(RTAX89U)
				if (!strcmp(p->name, "linux") && v->reserved_pebs == 529) {
					resize_linux_volume(ubi, v, p);
				}
#endif
			}
			continue;
...
int ranand_block_isbad(loff_t offs)
{
	...
	return mtd->_block_isbad(mtd, offs);
}
...
int ranand_write(unsigned char *buf, unsigned int to, int datalen)
{
	...
	r = mtd->_write(mtd, to, datalen, &retlen, buf);
	if (r < 0)
		return r;
...
int ranand_read(unsigned char *buf, unsigned int from, int datalen)
{
	...
	r = mtd->_read(mtd, from, datalen, &retlen, buf);
	if (r < 0)
		return r;
...
int ra_flash_init_layout(void)
{
#if defined(CONFIG_UBI_SUPPORT)
	int r, i, delta = 0;
	ulong mac_offset = RAMAC0_OFFSET;
	unsigned char mac[6], *m;
	char mac_str[sizeof("00:11:22:33:44:55XXX")];
	char ename[sizeof("ethXaddrXXXXXX")];
	struct ubi_device *ubi;
	char *ubi_part[] = { "ubi", "part", "UBI_DEV" };
	char *ubi_info_layout[] = { "ubi", "info", "wlayout" };

	eeprom_set = malloc(CFG_UBI_FACTORY_SIZE);
	if (!eeprom_set)
		printf("Allocate %d bytes memory for eeprom_set fail!\n", CFG_UBI_FACTORY_SIZE);
	...
	r = choose_active_eeprom_set();
#if defined(CONFIG_DUAL_BAND)
	mac_offset = RAMAC1_OFFSET;
	delta = 1;
#endif
	/* Fix ethaddr, ethXaddr in U-Boot environment if we have good Factory volume. */
	if (r == 0 &&!ra_factory_read(mac, mac_offset, 6)) {
		m = &mac[0];
		*(m + 5) += delta;	/* LAN MAC = 5G MAC + 1 */
		snprintf(mac_str, sizeof(mac_str), "%02X:%02X:%02X:%02X:%02X:%02X",
			*m, *(m + 1), *(m + 2), *(m + 3), *(m + 4), *(m + 5));
		for (i = 0; i < 6; ++i) {
			if (i)
				snprintf(ename, sizeof(ename), "eth%daddr", i);
			else
				snprintf(ename, sizeof(ename), "%s", "ethaddr");
			if (!getenv(ename))
				continue;
 ...
 * 		length of the EEPROM set must greater than or equal to CFG_UBI_FACTORY_SIZE
 * @return:
...
int choose_active_eeprom_set(void)
{
	...
	for (i = 0, o = 0; i < NR_EEPROM_SETS; ++i, o += CFG_UBI_FACTORY_SIZE) {
		r = ra_flash_read(buf, CFG_FACTORY_ADDR + o, CFG_UBI_FACTORY_SIZE);
		if (r) {
			printf("Read data fail at 0x%x\n", i * CFG_UBI_FACTORY_SIZE);
			continue;
		}
	...
	factory_offset = active_set * CFG_UBI_FACTORY_SIZE;
	r = ra_flash_read(eeprom_set, CFG_FACTORY_ADDR + factory_offset, CFG_UBI_FACTORY_SIZE);
...
static int update_eeprom_sets(void)
{
	...
	for (o = 0; o < CFG_UBI_FACTORY_SIZE; o += CFG_UBI_FACTORY_SIZE)
		memcpy(buf + o, eeprom_set, CFG_UBI_FACTORY_SIZE);
	for (o = 0; o < (NR_EEPROM_SETS * CFG_UBI_FACTORY_SIZE); o += CFG_UBI_FACTORY_SIZE) {
...
static int remove_software_marked_bad_block(void)
{
	...
	for (off = 0; off < nand->size; off += nand->erasesize) {
		...
		if ((ret = nand->_read_oob(nand, off, &ops)) != 0) {
		...
		if ((ret = nand->_read_oob(nand, off + nand->writesize, &ops)) != 0) {
...
int ra_factory_erase_write(uchar *buf, ulong off, ulong len, int prot)
{
	   ...
	   )
	{
		...
		if (in_range(off, len, QC98XX_EEPROM1_OFFSET, QC98XX_EEPROM_SIZE_LARGEST)) {
			upadte_wifi_eeprom_csum((void*)factory_buf + QC98XX_EEPROM1_OFFSET);
		}
#if !defined(CONFIG_ARCH_IPQ807x)
		if (in_range(off, len, QC98XX_EEPROM2_OFFSET, QC98XX_EEPROM_SIZE_LARGEST)) {
			upadte_wifi_eeprom_csum((void*)factory_buf + QC98XX_EEPROM2_OFFSET);
		}
#endif
 ...
 * 		length of the buffer must greather than or equal to CFG_UBI_FACTORY_SIZE.
 * @return:
...
{
	int r;
	unsigned char *mac = factory + (RAMAC0_OFFSET & 0xFFF);
	...
	if ((r = ra_flash_read(factory, CFG_FACTORY_ADDR, CFG_UBI_FACTORY_SIZE)) != 0) {
...
int __SolveUBI(unsigned char *ptr, unsigned int start_offset, unsigned int copysize)
{
	...
	unsigned char factory_raw_data[CFG_UBI_FACTORY_SIZE];
...
retry_solve_ubi:
	...
	if ((r = ra_flash_erase_write(factory_raw_data, CFG_FACTORY_ADDR, CFG_UBI_FACTORY_SIZE, 0)) != 0) {
