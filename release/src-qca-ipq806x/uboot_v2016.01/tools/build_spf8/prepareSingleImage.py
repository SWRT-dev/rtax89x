...
def gen_part(flash):
	...
	for flash_type in flash.split(","):
		prc = subprocess.Popen(['python2', flash_partition_path, '-c', configDir, '-f', flash_type, '-o', inDir], cwd=cdir)
		prc.wait()
...
def gen_bootconfig():
	...
	prc = subprocess.Popen(['python2', bootconfig_path, '-c', configDir, '-o', inDir], cwd=cdir)
	prc.wait()
...
def gen_mbn():
	...
	prc = subprocess.Popen(['python2', bootconfig_path, '-f', inDir + "/../../u-boot", '-o', inDir + "/openwrt-ipq807x-u-boot.mbn"], cwd=cdir)
	prc.wait()
