...
int get_eth_mac_address(uchar *enetaddr, uint no_of_macs)
{
	...
	r = ra_factory_read(mac, RAMAC0_OFFSET, 6);
	if (!r && !(mac[0] & 1) && memcmp(mac, empty_mac, 6) && memcmp(mac, zero_mac, 6)) {
		for (i = 0; i < no_of_macs; ++i, enetaddr+=6)
			memcpy(enetaddr, mac, 6);
		return 0;
	}

	/* It is not possible read MAC address from Factory here.
	 * Because UBI have not beed initialized yet.
	 * Always uses default MAC address here.
	 */
