 ...
 *   0 on success
 */
int miiphy_read(const char *devname, unsigned char addr, unsigned int reg,
		 ...
		 unsigned char *model, unsigned char *rev)
{
	...
	if (reg == 0xFFFF) {
		/* No physical device present at this address, test AQR PHY. */
		if (miiphy_read(devname, addr, MII_ADDR_C45 | (7 << 16) | MII_PHYSID1, &tmp) || tmp < 0)
			return -1;
		reg = tmp << 16;
	} else {
