...
{
	int i, trx_len = -1;
	char addr[] = "ffffffffXXX", *load_trx[] = { NULL, addr };
	char *boot_argv[] = { NULL, addr };
	unsigned char buf1[4];
	unsigned long kaddr[] = { CFG_KERN_ADDR, cfg_kern2_addr };

	for (i = 0; i < ARRAY_SIZE(kaddr); ++i) {
		snprintf(addr, sizeof(addr), "%lx", kaddr[i]);
		trx_len = check_trx(ARRAY_SIZE(load_trx), load_trx);
		if (trx_len <= CFG_BOOTLOADER_SIZE) {
			printf("Firmware-%d damaged!\n", i + 1);
			continue;
		}
	...
	ubi_damaged = 0;
	if (ra_flash_read(buf1, CFG_KERN_ADDR, 4) == -ENOENT)
		ubi_damaged = 1;
	printf(" \nHello!! Enter Recuse Mode: (Check error)\n\n");
	if (net_loop(TFTPD) < 0)
		return 1;
