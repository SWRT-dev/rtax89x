...
static int check_clear_crashdump_magic(void)
{
	...
	run_command("reset", 0);
#else
#error FIX crashdump magic handling!
#endif
...
int reset_to_default(void)
{
	...
	addr = CONFIG_SYS_FLASH_BASE + CFG_BOOTLOADER_SIZE + CONFIG_ENV_SIZE;
	size = CFG_NVRAM_SIZE;
...
#else
		ra_flash_erase_write((uchar*)load_address, CFG_KERN_ADDR, net_boot_file_size, 0);
#endif
	}
