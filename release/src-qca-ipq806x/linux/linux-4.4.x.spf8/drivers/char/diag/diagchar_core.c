...
static int diag_probe(struct platform_device *pdev)
{
	...
	mutex_init(&driver->diagfwd_channel_mutex[0]);
	mutex_init(&driver->diagfwd_channel_mutex[1]);
	mutex_init(&driver->diagfwd_channel_mutex[2]);
	mutex_init(&driver->diagfwd_channel_mutex[3]);
