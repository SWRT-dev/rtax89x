...
static inline int qca_thermal_trip_type(enum thermal_trip_type type)
{
				    ...
				    enum thermal_trip_type *type)
{
	...
	if (trip >= data->ntrips || trip < 0)
		return -EDOM;
	if (data->mode == THERMAL_DEVICE_DISABLED)
		return qca_thermal_trip_type(data->trips[trip].type)? -EDOM : 0;
				    ...
				    int temp)
{
	...
	if (data->ops->set_trip_temp) {
		...
		if (qca_thermal_trip_type(data->trips[trip].type)) {
