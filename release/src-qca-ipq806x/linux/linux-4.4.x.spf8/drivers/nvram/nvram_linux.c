...
nvram_commit_wrapped:
	...
	for (offset = unit_offset, data = (uint8_t*) header; offset < end_offset && total_wlen > 0; offset = next_erase_offset) {
		...
		d_len = wlen = min(next_erase_offset - offset, total_wlen);
		if (unlikely(!d_len)) {
			ret = -EINVAL;
			goto done;
		}
		...
		if (offset > prev_erase_offset) {
			...
			if (ret || len != rlen) {
				printk(KERN_WARNING "%s: backup data at 0x%x ret %d, len 0x%x/%x\n",
...
start_erase_write:
		...
		if (!ret && len == wlen) {
			...
			printk("%s: write to offset 0x%x error. ret %d len 0x%x/%x\n", __func__, o, ret, len, wlen);
			if (++w_retry <= 3) {
				need_erase = 1;
				goto start_erase_write;
			}
