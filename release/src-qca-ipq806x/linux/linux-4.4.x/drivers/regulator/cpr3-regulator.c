...
static irqreturn_t cpr3_irq_handler(int irq, void *data)
{
	...
	if (new_volt > aggr->ceiling_volt) {
		...
		local_irq_restore(flags);
		commit = 1;
		cpr3_warning(ctrl, "limiting to ceiling=%d uV\n",
			aggr->ceiling_volt);
		...
		local_irq_restore(flags);
		cpr3_warning(ctrl, "limiting to floor=%d uV\n", aggr->floor_volt);
	}
	...
	vreg = &ctrl->thread[0].vreg[0];
	if (vreg->last_corner == vreg->current_corner) {
		if (up) {
			local_irq_save(flags);
			if (time_after(jiffies, ctrl->last_volt_dn_t2)) {
				ctrl->volt_up++;
				commit = 1;
			}
			local_irq_restore(flags);
			if (commit)
				cpr3_warning(ctrl, "UP: new_volt=%d uV, last_volt=%d uV\n", new_volt, last_volt);
		} else if (down) {
			local_irq_save(flags);
			ctrl->volt_down++;
			ctrl->last_volt_dn_t2 = jiffies + 10 * HZ;
			local_irq_restore(flags);
			cpr3_debug(ctrl, "DN: new_volt=%d uV, last_volt=%d uV\n", new_volt, last_volt);
		}
	} else {
		cpr3_debug(ctrl, "%s: new_volt=%d uV, last_volt=%d uV\n",
			up ? "UP" : "DN", new_volt, last_volt);
		vreg->last_corner = vreg->current_corner;
	}
	...
	if (commit && !ctrl->stop_wq) {
		cancel_delayed_work(&ctrl->vstat_work);
		schedule_delayed_work(&ctrl->vstat_work, 20 * HZ);
	}
...
static int cpr3_regulator_init_vreg_data(struct cpr3_regulator *vreg)
{
	...
	vreg->last_corner = CPR3_REGULATOR_CORNER_INVALID;
