...
static int offset_mV[IPQ807x_APSS_FUSE_CORNERS] = { 0 };
...
static ssize_t limited_to_ceiling_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct cpr3_controller *ctrl = platform_get_drvdata(pdev);

	return sprintf(buf, "%u\n", ctrl->limited_to_ceiling);
}

static ssize_t limited_to_floor_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct cpr3_controller *ctrl = platform_get_drvdata(pdev);

	return sprintf(buf, "%u\n", ctrl->limited_to_floor);
}

static ssize_t volt_up_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct cpr3_controller *ctrl = platform_get_drvdata(pdev);
...
static ssize_t volt_down_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct cpr3_controller *ctrl = platform_get_drvdata(pdev);
...
static ssize_t stop_wq_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct platform_device *pdev = container_of(dev, struct platform_device, dev);
	struct cpr3_controller *ctrl = platform_get_drvdata(pdev);
...
static ssize_t stop_wq_store(struct device *dev, struct device_attribute *attr,
			  const char * buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &value) == 1) {
		ctrl->stop_wq = (value == 8916)? 1 : 0;
		if (ctrl->stop_wq)
			cancel_delayed_work_sync(&ctrl->vstat_work);
		else
			schedule_delayed_work(&ctrl->vstat_work, 0);
	}

	return n;
}

static const struct device_attribute cpr4_apss_attrs[] = {
	__ATTR_RO(limited_to_ceiling),
	__ATTR_RO(limited_to_floor),
	__ATTR_RO(volt_up),
	__ATTR_RO(volt_down),
	__ATTR_RW(stop_wq),
};

#define MTD_FACTORY_BASE_ADDRESS	(0)
...
/* Data structure that is used to load/save statistics information from/to Factory. */
struct vstat_s {
	uint64_t volt_up;		/* summary of initial/latest volt_up. */
	uint32_t l2ceiling;		/* summary of initial/latest limited_to_ceiling. */
	uint32_t pwr_cycle_cnt;		/* latest power cycle count. */
...
static void update_vstat_func(struct work_struct *work)
{
	...
	ctrl = container_of(to_delayed_work(work), struct cpr3_controller, vstat_work);
	if (ctrl->stop_wq)
		return;

	if (!ctrl->mtd) {
		ctrl->mtd = get_mtd_device_nm("Factory");
		if (IS_ERR(ctrl->mtd)) {
			schedule_delayed_work(&ctrl->vstat_work, 2 * HZ);
			return;
		}

		/* Load initial value from Factory */
		err = mtd_read(ctrl->mtd, OFFSET_VOLTUP, sizeof(vs), &retlen, (u_char *) &vs);
		if (!err && retlen == sizeof(vs)) {
			if (vs.volt_up == U64_MAX)
				vs.volt_up = 0;
			if (vs.l2ceiling == U32_MAX)
				vs.l2ceiling = 0;
			if (vs.pwr_cycle_cnt == U32_MAX)
				vs.pwr_cycle_cnt = 0;
			if (vs.avg_uptime == U32_MAX)
				vs.avg_uptime = 0;

			ctrl->last_volt_up = ctrl->loaded_volt_up = vs.volt_up;
			ctrl->last_l2ceiling = ctrl->loaded_l2ceiling = vs.l2ceiling;
			ctrl->last_pwr_cycle_cnt = ctrl->loaded_pwr_cycle_cnt = ctrl->pwr_cycle_cnt = vs.pwr_cycle_cnt;
			ctrl->loaded_avg_time = ctrl->last_avg_uptime = vs.avg_uptime;
			printk("Load vstat C:%u U:%llu P:%u T:%u\n", ctrl->last_l2ceiling,
				ctrl->last_volt_up, ctrl->loaded_pwr_cycle_cnt, ctrl->last_avg_uptime);
	...
	spin_lock_irqsave(&ctrl->vstat_lock, flags);
	if (U64_MAX - ctrl->volt_up > ctrl->loaded_volt_up)
		vs.volt_up = ctrl->volt_up + ctrl->loaded_volt_up;
	else
		vs.volt_up = U64_MAX - 1;
	if (U32_MAX - ctrl->limited_to_ceiling > ctrl->loaded_l2ceiling)
		vs.l2ceiling = ctrl->limited_to_ceiling + ctrl->loaded_l2ceiling;
	else
		vs.l2ceiling = U32_MAX - 1;
	spin_unlock_irqrestore(&ctrl->vstat_lock, flags);

	get_monotonic_boottime(&t);
	total_uptime = (ctrl->loaded_avg_time * ctrl->loaded_pwr_cycle_cnt) + t.tv_sec;
	do_div(total_uptime, ctrl->pwr_cycle_cnt);

	vs.pwr_cycle_cnt = ctrl->pwr_cycle_cnt;
	vs.avg_uptime = (ctrl->last_avg_uptime < U32_MAX)? ctrl->last_avg_uptime : U32_MAX - 1;

	/* Update statistics information to Factory if one of below conditions true:
	 * 1. l2ceiling changed.
	 * 2. Per 1 hour in first 24 hours and per 12 hours in rest if volt_up changed..
	 * 3. power cycle count updated.
	 * 4. Per 1 hour in first 24 hours and per 12 hours in rest if average uptime changed.
	 */
	interval = (t.tv_sec > 86400)? 86400 / 2 : 60 * 60;
	if (ctrl->last_l2ceiling != vs.l2ceiling
	 || (ctrl->last_volt_up != vs.volt_up && (t.tv_sec - ctrl->t1.tv_sec > interval))
	 || ctrl->last_pwr_cycle_cnt != ctrl->pwr_cycle_cnt
	 || (ctrl->last_avg_uptime != total_uptime && ctrl->t1.tv_sec && t.tv_sec - ctrl->t1.tv_sec > interval)) {

		err = mtd_write(ctrl->mtd, OFFSET_VOLTUP, sizeof(vs), &retlen, (u_char *) &vs);
		if (err || retlen != sizeof(vs)) {
			printk(KERN_WARNING "Update vstat failed. (err %d retlen %u)\n", err, retlen);
		} else {
			ctrl->last_l2ceiling = vs.l2ceiling;
			ctrl->last_volt_up = vs.volt_up;
			ctrl->last_pwr_cycle_cnt = ctrl->pwr_cycle_cnt;
			...
			struct cpr3_regulator *vreg)
{
	...
	int *fuse_volt, *misc_adj_volt, prev_volt;
	int *fmax_corner;
	...
	for (i = 0; i < vreg->fuse_corner_count; i++) {
		...
		cpr3_notice(vreg, "fused %8s: open-loop=%7d uV\n",
	...
	/* Add offset_mV[] to fuse_volt. */
	for (i = 0; i < vreg->fuse_corner_count && i < ARRAY_SIZE(offset_mV); i++) {
		if (!offset_mV[i])
			continue;
		prev_volt = fuse_volt[i];
		fuse_volt[i] += offset_mV[i] * 1000;
		cpr3_debug(vreg, "corner %d open-loop voltage: %d --> %d uV\n",
...
static void cpr4_apss_print_settings(struct cpr3_regulator *vreg)
{
	...
	cpr3_warning(vreg, "Corner: Frequency (Hz), Fuse Corner, Floor (uV), Open-Loop (uV), Ceiling (uV)\n");
	for (i = 0; i < vreg->corner_count; i++) {
		corner = &vreg->corner[i];
		cpr3_warning(vreg, "%3d: %10u, %2d, %7d, %7d, %7d\n",
...
static ssize_t offset_mv_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	return sprintf(buf, "%d %d %d %d\n", offset_mV[0], offset_mV[1], offset_mV[2], offset_mV[3]);
}

static ssize_t offset_mv_store(struct device *dev, struct device_attribute *attr,
			  const char * buf, size_t n)
{
	int i, r;
	u32 *temp;
	int value[IPQ807x_APSS_FUSE_CORNERS];
	struct platform_device *pdev;
	struct cpr3_controller *ctrl;
	struct cpr3_regulator *vreg;
	struct cpr3_corner *corner;

	r = sscanf(buf, "%d %d %d %d", &value[0], &value[1], &value[2], &value[3]);
	if (r <= 0 || r > ARRAY_SIZE(offset_mV))
		return -EINVAL;
	/* Reject all value if one of them is illegal. */
	for (i = 0; i < r; ++i) {
		if (value[i] < -40 || value[i] > 100)
			return -EINVAL;
	}
	for (i = 0; i < r; ++i)
		offset_mV[i] = value[i];
	...
	for (i = 0; i < ctrl->thread[0].vreg_count; i++) {
		vreg = &ctrl->thread[0].vreg[i];

		temp = kcalloc(vreg->corner_count, sizeof(*temp), GFP_KERNEL);
		if (!temp)
			return -ENOMEM;

		/* Reload floor_volt defined in .dts due to cpr3_limit_floor_voltages() may change it. */
		r = cpr3_parse_corner_array_property(vreg, "qcom,cpr-voltage-floor", 1, temp);
		for (i = 0; !r && i < vreg->corner_count; i++) {
			vreg->corner[i].floor_volt = CPR3_ROUND(temp[i], ctrl->step_volt);
		}

		/* Reload ceiling_volt defined in .dts if it's override by last calculated open_loop_volt at
		 * system start-up due to qcom,cpr-scaled-open-loop-voltage-as-ceiling is defined in .dts
		 */
		if (of_property_read_bool(vreg->of_node, "qcom,cpr-scaled-open-loop-voltage-as-ceiling")) {
			r = cpr3_parse_corner_array_property(vreg, "qcom,cpr-voltage-ceiling", 1, temp);
			for (i = 0; !r && i < vreg->corner_count; i++) {
				vreg->corner[i].ceiling_volt = CPR3_ROUND(temp[i], ctrl->step_volt);
				vreg->corner[i].abs_ceiling_volt = vreg->corner[i].ceiling_volt;
			}
		}
		kfree(temp);

		r = cpr4_ipq807x_apss_calculate_open_loop_voltages(vreg);
		if (r) {
			cpr3_err(vreg, "unable to calculate open-loop voltages, r=%d\n", r);
			return r;
		}

		r = cpr3_limit_open_loop_voltages(vreg);
		if (r) {
			cpr3_err(vreg, "unable to limit open-loop voltages, r=%d\n", r);
			return r;
		}

		cpr3_open_loop_voltage_as_ceiling(vreg);

		r = cpr3_limit_floor_voltages(vreg);
		if (r) {
			cpr3_err(vreg, "unable to limit floor voltages, r=%d\n", r);
			return r;
		}

		r = cpr4_ipq807x_apss_calculate_target_quotients(vreg);
		if (r) {
			cpr3_err(vreg, "unable to calculate target quotients, r=%d\n", r);
			return r;
		}

		r = cpr4_parse_core_count_temp_voltage_adj(vreg, false);
		if (r) {
			cpr3_err(vreg, "unable to parse temperature and core count voltage adjustments, r=%d\n", r);
			return r;
		}

		if (vreg->allow_core_count_adj
		 && (vreg->max_core_count <= 0
		  || vreg->max_core_count > IPQ807x_APSS_CPR_SDELTA_CORE_COUNT)) {
			cpr3_err(vreg, "qcom,max-core-count has invalid value = %d\n", vreg->max_core_count);
			return -EINVAL;
		}

		r = cpr4_apss_parse_boost_properties(vreg);
		if (r) {
			cpr3_err(vreg, "unable to parse boost adjustments, r=%d\n", r);
			return r;
		}
		cpr4_apss_print_settings(vreg);
		...
		if (!ctrl->use_hw_closed_loop) {
			for (i = 0; i < vreg->corner_count; ++i) {
				corner = &vreg->corner[i];
				corner->last_volt = corner->open_loop_volt;
			}
		}
		r = cpr3_regulator_update_ctrl_state(ctrl);
		if (r) {
			cpr3_err(vreg, "could not update CPR state, r=%d\n", r);
			return r;
		}
...
static DEVICE_ATTR_RW(offset_mv);
...
static int cpr4_apss_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(cpr4_apss_attrs); i++) {
		rc = device_create_file(dev, &cpr4_apss_attrs[i]);
		if (rc) {
			pr_err("%s: device_create_file failed: %d\n", __func__, rc);
		}
	}

	rc = sysfs_create_file(&pdev->dev.kobj, &dev_attr_offset_mv.attr);
	if (rc)
		pr_err("%s: sysfs_create_file failed: %d\n", __func__, rc);
...
static int cpr4_apss_regulator_remove(struct platform_device *pdev)
{
	...
	cancel_delayed_work_sync(&ctrl->vstat_work);
	if (ctrl->mtd)
		put_mtd_device(ctrl->mtd);

	sysfs_remove_file(&pdev->dev.kobj, &dev_attr_offset_mv.attr);
	for (i = 0; i < ARRAY_SIZE(cpr4_apss_attrs); i++) {
		device_remove_file(dev, &cpr4_apss_attrs[i]);
	}
