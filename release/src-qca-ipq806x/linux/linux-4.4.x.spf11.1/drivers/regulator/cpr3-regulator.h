...
struct cpr3_regulator {
	...
	int			last_corner;	/* used in cpr3_irq_handler() only */
};
...
struct cpr3_controller {
	...
	struct mtd_info		*mtd;
	int			stop_wq;		/* disable workqueue */
	uint64_t		loaded_volt_up;		/* initial loaded volt_up in Factory */
	uint32_t		loaded_l2ceiling;	/* initial loaded limited_to_ceiling in Factory */
	uint32_t		loaded_pwr_cycle_cnt;	/* initial loaded power_cycle_cnt in Factory */
	uint32_t		loaded_avg_time;	/* initial loaded avg_time in Factory, unit: seconds */
	uint64_t		last_volt_up;		/* latest summary value */
	uint32_t		last_l2ceiling;		/* latest summary value */
	uint32_t		last_pwr_cycle_cnt;	/* latest summary value */
	uint32_t		last_avg_uptime;	/* latest average uptime */
	uint32_t		pwr_cycle_cnt;
	...
	unsigned int		volt_up;		/* number of CPR3_IRQ_UP interrupt */
	unsigned int		volt_down;		/* number of CPR3_IRQ_DOWN interrupt */
};
...
#define cpr3_warning(cpr3_thread, message, ...) \
	pr_warning("%s: " message, (cpr3_thread)->name, ##__VA_ARGS__)
#define cpr3_notice(cpr3_thread, message, ...) \
	pr_notice("%s: " message, (cpr3_thread)->name, ##__VA_ARGS__)
...
int cpr3_regulator_update_ctrl_state(struct cpr3_controller *ctrl);
