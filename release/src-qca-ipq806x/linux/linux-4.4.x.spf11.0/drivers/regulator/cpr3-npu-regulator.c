...
static int offset_mV[IPQ807x_NPU_FUSE_CORNERS] = { 0 };
...
static int cpr3_ipq807x_npu_read_fuse_data(struct cpr3_regulator *vreg)
{
	...
	for (i = 0; i < g_valid_npu_fuse_count; i++) {
		...
		cpr3_notice(vreg, "fuse->init_voltage[%d] = %llu\n", i, fuse->init_voltage[i]);
	}
			...
			struct cpr3_regulator *vreg, bool temp_correction)
{
	...
	if (rc == 0) {
		...
		for (i = 0; i < vreg->corner_count; i++)
			cpr3_notice(vreg, "open-loop[%2d] = %d uV\n", i,
...
{
	int i, r;
	int value[IPQ807x_NPU_FUSE_CORNERS];
	struct platform_device *pdev;
	struct cpr3_controller *ctrl;
	struct cpr3_regulator *vreg;
	bool can_adj_cold_temp;

	r = sscanf(buf, "%d %d", &value[0], &value[1]);
	...
	for (i = 0; i < ctrl->thread[0].vreg_count; i++) {
		...
		can_adj_cold_temp = cpr3_can_adjust_cold_temp(vreg);

		if (can_adj_cold_temp) {
			r = cpr3_ipq807x_npu_calc_temp_based_ol_voltages(vreg, true);
			if (r) {
		...
		r = cpr3_ipq807x_npu_calc_temp_based_ol_voltages(vreg, false);
		if (r) {
			cpr3_err(vreg, "unable to calculate open-loop voltages, r=%d\n", r);
			return r;
		}
