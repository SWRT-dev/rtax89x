...
static void update_vstat_func(struct work_struct *work)
{
	...
	if (!ctrl->mtd) {
		...
		if (!err && retlen == sizeof(vs)) {
			...
			ctrl->loaded_pwr_cycle_cnt = ctrl->pwr_cycle_cnt = vs.pwr_cycle_cnt;
	 ...
	 * 1. volt_up or l2ceiling changed.
	 * 2. power cycle count updated.
	 * 3. Per 1 hour in first 24 hours and per 12 hours in rest if average uptime changed.
	 */
	interval = (t.tv_sec > 86400)? 86400 / 2 : 60 * 60;
	if (ctrl->last_l2ceiling != vs.l2ceiling
	 || ctrl->last_volt_up != vs.volt_up
	 || ctrl->loaded_pwr_cycle_cnt != ctrl->pwr_cycle_cnt
