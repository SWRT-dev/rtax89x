...
static MTLAN_T *_wg_get_mtlan(wg_type_t type, int unit, MTLAN_T *pmtl, size_t *pmtl_sz)
{
	SDNIDX_TYPE sdntype;
	int idx;

	if (type == WG_TYPE_CLIENT) {
		sdntype = SDNIDX_TYPE_VPNC;
		idx = get_vpnc_idx_by_proto_unit(VPN_PROTO_WG, unit);
	}
	else {
		sdntype = SDNIDX_TYPE_VPNS;
		idx = get_vpns_idx_by_proto_unit(VPN_PROTO_WG, unit);
	}

	if (!get_mtlan_by_idx(sdntype, idx, pmtl, pmtl_sz)) {
		FREE_MTLAN((void *)pmtl);
		_dprintf("get mtlan by idx(%d) failed!!!\n", idx);
		return NULL;
	}
	else {
		// _dprintf("mtlan number: %u\n", mtl_sz);
		return pmtl;
	}
}

static char* _wg_get_lan_ifnames(wg_type_t type, int unit, char* buf, size_t len)
{
	MTLAN_T *pmtl = (MTLAN_T *)INIT_MTLAN(sizeof(MTLAN_T));
	size_t mtl_sz = 0;
	int i;

	if (_wg_get_mtlan(type, unit, pmtl, &mtl_sz)) {
		memset(buf, 0, len);
		for (i = 0; i < mtl_sz; i++) {
...
void write_wgs_fw_filter_input(FILE* fp)
{
	int unit;
	char prefix[16] = {0};
	char ifname[8] = {0};
...
void write_wgs_fw_filter_forward(FILE* fp)
{
	int unit;
	char prefix[16] = {0};
	char ifname[8] = {0};
	...
	for(unit = 1; unit <= WG_SERVER_MAX; unit++)
	{
		...
		if (nvram_pf_get_int(prefix, "enable"))
		{
			///NOTE: need different routing table for different lan, otherwise need adding -i itf -o wgsX for different lan.
			fprintf(fp, "-A FORWARD -o %s -j ACCEPT\n", ifname);
