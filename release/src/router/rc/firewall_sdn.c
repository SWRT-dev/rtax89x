...
int update_SDN_iptables(const MTLAN_T *pmtl, const char *logdrop, const char *logaccept)
{
	...
	if (fp)
	{
		...
		if (pmtl->enable)
		{
			...
			//WAN rules
			if(pmtl->sdn_t.vpnc_idx)
			{
				snprintf(tmp, sizeof(tmp), "vpnc%d_ifname", pmtl->sdn_t.vpnc_idx);
				vpnc_if = nvram_safe_get(tmp);
				if(vpnc_if && vpnc_if[0] != '\0')
					fprintf(fp, "-I %s -i %s -o %s -j %s\n", SDN_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, vpnc_if, logaccept);
			}
//TODO wan_idx
			...
			if (!(pmtl->sdn_t.killsw_sw))
			{
				...
				for (i = WAN_UNIT_FIRST; i < wan_max_unit; ++i)
				{
					...
					fprintf(fp, "-I %s -i %s -o %s -j %s\n", SDN_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, wan_if, logaccept);
				}
#else
				fprintf(fp, "-I %s -i %s -o %s -j %s\n", SDN_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, wan_if, logaccept);
#endif
	...
	if (ipv6_enabled())
	{
		...
		if (fp)
		{
			...
			if (pmtl->enable)
			{
				...
				if (pmtl->sdn_t.killsw_sw)
				{
					fprintf(fp, "-I %s -i %s -o %s -j %s\n", SDN_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, wan6face, logdrop);
				}
			}
			fprintf(fp, commit_str);
			fclose(fp);
...
#ifdef RTCONFIG_IPV6
static int _write_UrlFilter(const int sdn_idx, const URLF_PF *urlf_pf, const char *chain, const char *ifname, const char *ip, const char *logdrop, FILE *fp, FILE *fp_ipv6)
#else
static int _write_UrlFilter(const int sdn_idx, const URLF_PF *urlf_pf, const char *chain, const char *ifname, const char *ip, const char *logdrop, FILE *fp)
#endif
{
	char srcips[64];
	char config_rule[20];
...
#endif
	snprintf(sdn_chain, sizeof(sdn_chain), "%s_%d", chain, sdn_idx);
...
#ifdef RTCONFIG_IPV6
int write_URLFilter_SDN(const MTLAN_T *pmtl, const int create_chain, const char *logdrop, FILE *fp_filter, FILE *fpv6_filter)
#else
int write_URLFilter_SDN(const MTLAN_T *pmtl, const int create_chain, const char *logdrop, FILE *fp_filter)
#endif
{
	URLF_PF urlf_pf;
	...
	if (create_chain)
	{
		fprintf(fp_filter, "-N %s_%d\n", URL_FILTER_INPUT_CHAIN, pmtl->sdn_t.sdn_idx);
		fprintf(fp_filter, "-N %s_%d\n", URL_FILTER_FORWARD_CHAIN, pmtl->sdn_t.sdn_idx);
#ifdef RTCONFIG_IPV6
		if (fpv6_filter)
		{
			fprintf(fpv6_filter, "-N %s_%d\n", URL_FILTER_INPUT_CHAIN, pmtl->sdn_t.sdn_idx);
			fprintf(fpv6_filter, "-N %s_%d\n", URL_FILTER_FORWARD_CHAIN, pmtl->sdn_t.sdn_idx);
		}
#endif
	}
...
#ifdef RTCONFIG_IPV6
	_write_UrlFilter(pmtl->sdn_t.sdn_idx, (pmtl->enable && pmtl->sdn_t.urlf_idx) ? &urlf_pf : NULL, URL_FILTER_INPUT_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp_filter, fpv6_filter);
	_write_UrlFilter(pmtl->sdn_t.sdn_idx, (pmtl->enable && pmtl->sdn_t.urlf_idx) ? &urlf_pf : NULL, URL_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp_filter, fpv6_filter);
#else
	_write_UrlFilter(pmtl->sdn_t.sdn_idx, (pmtl->enable && pmtl->sdn_t.urlf_idx) ? &urlf_pf : NULL, URL_FILTER_INPUT_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp_filter);
	_write_UrlFilter(pmtl->sdn_t.sdn_idx, (pmtl->enable && pmtl->sdn_t.urlf_idx) ? &urlf_pf : NULL, URL_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp_filter);
#endif
...
int handle_URLFilter_jump_rule(const MTLAN_T *pmtl)
{
	...
	doSystem("iptables -D %s -p udp -m udp --dport 53 -j %s", URL_FILTER_INPUT_CHAIN, sdn_chain);
	...
	doSystem("iptables -D %s -j %s", URL_FILTER_FORWARD_CHAIN, sdn_chain);
	if (pmtl->enable && _check_urlf_pf_enable(pmtl->sdn_t.urlf_idx))
	{
	...
	{
		snprintf(sdn_chain, sizeof(sdn_chain), "%s_%d", URL_FILTER_INPUT_CHAIN, pmtl->sdn_t.sdn_idx);
		doSystem("ip6tables -D %s -p udp -m udp --dport 53 -j %s", URL_FILTER_INPUT_CHAIN, sdn_chain);
		...
		snprintf(sdn_chain, sizeof(sdn_chain), "%s_%d", URL_FILTER_FORWARD_CHAIN, pmtl->sdn_t.sdn_idx);
		doSystem("ip6tables -D %s -j %s", URL_FILTER_FORWARD_CHAIN, sdn_chain);
		if (pmtl->enable && _check_urlf_pf_enable(pmtl->sdn_t.urlf_idx))
		{
...
#ifdef RTCONFIG_IPV6
				_write_UrlFilter(pmtl[i].sdn_t.sdn_idx, &urlf_pf, URL_FILTER_INPUT_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp, fpv6);
				_write_UrlFilter(pmtl[i].sdn_t.sdn_idx, &urlf_pf, URL_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp, fpv6);
#else
				_write_UrlFilter(pmtl[i].sdn_t.sdn_idx, &urlf_pf, URL_FILTER_INPUT_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp);
				_write_UrlFilter(pmtl[i].sdn_t.sdn_idx, &urlf_pf, URL_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, pmtl->nw_t.addr, logdrop, fp);
#endif
...
#ifdef RTCONFIG_IPV6
int write_NwServiceFilter_SDN(const MTLAN_T *pmtl, const int create_chain, const char *logdrop, const char *logaccept, FILE *fp_filter, FILE *fpv6_filter)
#else
int write_NwServiceFilter_SDN(const MTLAN_T *pmtl, const int create_chain, const char *logdrop, const char *logaccept, FILE *fp_filter)
#endif
{
	NWF_PF nwf_pf;
	NWF_RULE *rule;
	...
	// flush chain first.
	fprintf(fp_filter, "-F %s\n", chain);

#ifdef RTCONFIG_IPV6
	if (fpv6_filter)
	{
		fprintf(fpv6_filter, "-F %s\n", chain);
	}
#endif
...
int handle_NwServiceFilter_jump_rule(const MTLAN_T *pmtl)
{
	...
	doSystem("iptables -D %s -i %s -j %s", NW_SERVICE_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, sdn_chain);
	...
	{
		snprintf(sdn_chain, sizeof(sdn_chain), "%s_%d", NW_SERVICE_FILTER_FORWARD_CHAIN, pmtl->sdn_t.sdn_idx);
		doSystem("ip6tables -D %s -i %s -j %s", NW_SERVICE_FILTER_FORWARD_CHAIN, pmtl->nw_t.ifname, sdn_chain);
		if (pmtl->enable && _check_nwf_pf_enable(pmtl->sdn_t.nwf_idx))
		{
...
#ifdef RTCONFIG_IPV6
				write_NwServiceFilter_SDN(&pmtl[i], 0, logdrop, logaccept, fp, fpv6);
#else
				write_NwServiceFilter_SDN(&pmtl[i], 0, logdrop, logaccept, fp);
#endif
