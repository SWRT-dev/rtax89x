...
static int _set_network_routing_rule(const VPNC_ROUTE_CMD cmd, const int vpnc_idx)
{
	...
	if (pmtl)
	{
		if (get_mtlan_by_idx(SDNFT_TYPE_VPNC, vpnc_idx, pmtl, &mtl_sz))
		{
			for (i = 0; i < mtl_sz; ++i)
			{
				if (pmtl[i].enable && pmtl[i].nw_t.ifname[0] != '\0')
				{
					if (cmd == VPNC_ROUTE_ADD)
					{
						eval("ip", "rule", "del", "priority", priority);
					}
					eval("ip", "rule", cmd_str, "iif", pmtl[i].nw_t.ifname, "table", id_str, "priority", priority);
				}
			}
		}
		else
...
#ifdef RTCONFIG_MULTILAN_CFG
int vpnc_set_iptables_rule_by_sdn(MTLAN_T *pmtl, size_t mtl_sz, int restart_all_sdn)
{
		...
		if (nvram_pf_get_int(vpnc_prefix, "state_t") == WAN_STATE_CONNECTED)
		{
			...
			if (proto == VPNC_PROTO_PPTP || proto == VPNC_PROTO_L2TP || proto == VPNC_PROTO_IPSEC)
			{
				for (i = 0; i < mtl_sz; i++)
				{
					...
					snprintf(fpath, sizeof(fpath), "%s/fw_%s_sdn%d.sh", sdn_dir, vpnc_ifname, pmtl[i].nw_t.idx);
					...
					if (!f_exists(fpath) && vpnc_idx == pmtl[i].sdn_t.vpnc_idx)
					{
						...
						if (fp)
						{
							...
							fprintf(fp, "iptables -A VPNCF -o %s -j DROP\n", vpnc_ifname);
							fclose(fp);
				...
				{
					snprintf(fpath, sizeof(fpath), "%s/fw_%s_sdn%d.sh", sdn_dir, vpnc_ifname, i);
					if (f_exists(fpath))
						sdn_rule_exist = 1;
				}
				snprintf(fpath, sizeof(fpath), "%s/fw_%s_none.sh", sdn_dir, vpnc_ifname);
				if (sdn_rule_exist)
				{
					if (f_exists(fpath))
					{
				...
				else
				{
					if (!f_exists(fpath))
					{
						...
						{
							fprintf(fp, "#!/bin/sh\n\n");
							fprintf(fp, "iptables -I VPNCF -i %s -j ACCEPT\n", vpnc_ifname);
							fprintf(fp, "iptables -I VPNCF -o %s -j ACCEPT\n", vpnc_ifname);
							fclose(fp);
							chmod(fpath, S_IRUSR|S_IWUSR|S_IXUSR);
							eval(fpath);
						}
