...
var _chanspecs_6g = JSON.parse('<% chanspecs_6g(); %>');
...
if(isSupport('wifi6e')){
	...
	wl_info['3'].dfs_support = (function(){
		if(_chanspecs_6g.indexOf('100') != -1){
			return true
		}

		return false;
	})();
	wl_info['3'].bw_160_support = (function(){
		var count = 0;
		for(i=0;i<_chanspecs_6g.length;i++){
			if(_chanspecs_6g[i].indexOf('/160') != -1){
				count++;
			}
		}
	...
	for(i=0;i<_chanspecs_6g.length;i++){
		if(_chanspecs_6g[i].indexOf("/80") != -1){
			...
			wl3.channel_20m.push(_chanspecs_6g[i]);
		}
	}	
}
	...
	if (phytype == "a") {	// a mode
		...
		if (band == "0"){ // ---- 5 GHz
			if(wl_channel_list_5g instanceof Array && wl_channel_list_5g != ["0"]){	//With wireless channel 5g hook or return not ["0"]
				wl_channel_list_5g = JSON.parse('<% channel_list_2g(); %>');			
			...
			if(bw_cap == "0"){	// 20/40/80/160 MHz, 20/40/80 MHz, 20/40 MHz (Auto)
					...
					for(j=1; j<mesh_6g.auto.chanspec.length; j++){
						if(band6g_support && document.getElementById('psc6g_checkbox').checked){
                            for(var k=wl_channel_list_6g.length-1; k>=0; k--){
                                var _str = '6g' + wl_channel_list_6g[k];
                                if(mesh_6g.auto.chanspec[j].search(_str) != -1){
					...
					loop_auto: for(i=0; i<wl_channel_list_6g.length; i++){
						...
						if(band5g_11ax_support && document.form.wl_nmode_x.value != 1 && bw_160_support && enable_bw_160){
							for(j=0;j<wl3.channel_160m.length;j++){
								if(wl3.channel_160m[j].indexOf(_cur_channel) != -1){
									if(band6g_support){
										wl_channel_list_6g[i] = "6g" + _cur_channel + "/160";
									}
									else{
										wl_channel_list_6g[i] = _cur_channel + "/160";
									}
						...
						if(band5g_11ac_support && document.form.wl_nmode_x.value != 1){
							for(j=0;j<wl3.channel_80m.length;j++){
								if(wl3.channel_80m[j].indexOf(_cur_channel) != -1){
									if(band6g_support){
										wl_channel_list_6g[i] = "6g" + _cur_channel + "/80";
									}
									else{
										wl_channel_list_6g[i] = _cur_channel + "/80";
									}
						...
						for(j=0;j<wl3.channel_40m.length;j++){
							if(wl3.channel_40m[j].indexOf(_cur_channel) != -1){
								if(band6g_support){
									wl_channel_list_6g[i] = "6g" + _cur_channel + "/40";
								}
								else{
									wl_channel_list_6g[i] = wlextchannel_fourty(_cur_channel);
								}
						...
						for(j=0;j<wl3.channel_20m.length;j++){
							if(wl3.channel_20m[j].indexOf(_cur_channel) != -1){
								if(band6g_support){
									wl_channel_list_6g[i] = "6g" +  _cur_channel;
								}
								else{
									wl_channel_list_6g[i] = _cur_channel;
								}
				...
				var _wl_channel = new Array();
				if(amesh_support && httpApi.hasAiMeshNode()){
					for(j=1; j<mesh_6g.chan_160m.chanspec.length; j++){
						if(band6g_support && document.getElementById('psc6g_checkbox').checked){
                            for(var k=wl_channel_list_6g.length-1; k>=0; k--){
                                var _str = '6g' + wl_channel_list_6g[k];
                                if(mesh_6g.chan_160m.chanspec[j].search(_str) != -1){
                                    _wl_channel.push(mesh_6g.chan_160m.chanspec[j]);
                                   break;
                                }
                            }             
                        }
					...
					for(i=0;i<wl_channel_list_6g.length; i++){
						...
						for(j=0;j<wl3.channel_160m.length;j++){
							if(band6g_support){
								...
								if(w3.channel_160m[j].match(_reg) != null){	
									_wl_channel.push(_cur_channel + "/160");															
								}
							}							
						}
				...
				var _wl_channel = new Array();
				if(amesh_support && httpApi.hasAiMeshNode()){
					for(j=1; j<mesh_6g.chan_80m.chanspec.length; j++){
						if(band6g_support && document.getElementById('psc6g_checkbox').checked){
                            for(var k=wl_channel_list_6g.length-1; k>=0; k--){
                                var _str = '6g' + wl_channel_list_6g[k];
                                if(mesh_6g.chan_80m.chanspec[j].search(_str) != -1){
                                    _wl_channel.push(mesh_6g.chan_80m.chanspec[j]);
                                   break;
                                }
                            }             
                        }
				...
				var _wl_channel = new Array();
				if(amesh_support && httpApi.hasAiMeshNode()){
					for(j=1; j<mesh_6g.chan_40m.chanspec.length; j++){
						if(band6g_support && document.getElementById('psc6g_checkbox').checked){
                            for(var k=wl_channel_list_6g.length-1; k>=0; k--){
                                var _str = '6g' + wl_channel_list_6g[k];
                                if(mesh_6g.chan_40m.chanspec[j].search(_str) != -1){
                                    _wl_channel.push(mesh_6g.chan_40m.chanspec[j]);
                                   break;
                                }
                            }             
                        }
					...
					for(i=0;i<wl_channel_list_6g.length; i++){
						...
						for(j=0;j<wl3.channel_40m.length;j++){
							if(band6g_support){
								...
								var _l = wl3.channel_40m[j].split("l");
								var _u = wl3.channel_40m[j].split("u");
								if(_l.length > 1){
									if(_l[0] == _cur_channel){
										_wl_channel.push(wlextchannel_fourty(_cur_channel));
									}
				...
				var _wl_channel = new Array();
				if(amesh_support && httpApi.hasAiMeshNode()){					
					for(j=1; j<mesh_6g.chan_20m.chanspec.length; j++){
						if(band6g_support && document.getElementById('psc6g_checkbox').checked){
                            for(var k=wl_channel_list_6g.length-1; k>=0; k--){
                                var _str = '6g' + wl_channel_list_6g[k];
                                if(mesh_6g.chan_20m.chanspec[j].search(_str) != -1){
                            ...
                            _wl_channel.push(mesh_6g.chan_20m.chanspec[j]);
                        }
					}
					...
					for(i=0;i<wl_channel_list_6g.length; i++){
						...
						for(j=0;j<wl3.channel_20m.length;j++){
							if(band6g_support){
								...
								if(wl3.channel_20m[j].match(_reg) != null){
			...
			/*add "Auto" into the option list*/	
			if(wl_channel_list_6g[0] != "0")
			wl_channel_list_6g.splice(0,0,"0");
			
			add_options_x2(document.form.wl_nctrlsb, extend_channel, extend_channel_value, 1);   //construct extension channel
			chanspecs = wl_channel_list_6g;
		}	
		else if(band == "1"){	// 5 GHz-2
