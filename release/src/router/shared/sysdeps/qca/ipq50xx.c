 ...
 *     -1:	invalid parameter
 */
int ipq50xx_vlan_set(int vtype, char *upstream_if, int vid, int prio, unsigned int mbr, unsigned int untag)
{
 ...
 *  otherwise:	fail
 */
static int get_ipq50xx_port_info(unsigned int port, unsigned int *link, unsigned int *speed)
{
        FILE *fp;
	size_t rlen;
 ...
 *  otherwise:	fail
 */
static int get_ipq50xx_vport_info(unsigned int vport, unsigned int *link, unsigned int *speed)
{
	int phy;
	...
	get_ipq50xx_port_info(phy, link, speed);
 ...
 */
static void get_ipq50xx_phy_linkStatus(unsigned int mask, unsigned int *linkStatus)
{
	int i;
	...
	for (i = 0; m > 0 && !value; ++i, m >>= 1) {
		...
		get_ipq50xx_vport_info(i, &value, NULL);
		value &= 0x1;
	}
	*linkStatus = value;
}
 ...
 * 	6:	LAN3+LAN4
 */
static void config_ipq50xx_LANWANPartition(int type)
{
...
static void get_ipq50xx_Port_Speed(unsigned int port_mask, unsigned int *speed)
{
	int i, v = -1, t;
	unsigned int m;
	...
	for (i = 0; m; ++i, m >>= 1) {
		if (!(m & 1))
			continue;
		get_ipq50xx_vport_info(i, NULL, (unsigned int*) &t);
...
static void get_ipq50xx_WAN_Speed(unsigned int *speed)
{
	get_ipq50xx_Port_Speed(get_wan_port_mask(0) | get_wan_port_mask(1), speed);
}
 ...
 */
static void link_down_up_ipq50xx_PHY(unsigned int vpmask, int status)
{
	int vport, phy;
	unsigned int m;
	char idx[3];
...
static void create_Vlan(int bitmask)
{
	...
	ipq50xx_vlan_set(vtype, upstream_if, vid, prio, mbr_qca, untag_qca);
}

int ipq50xx_ioctl(int val, int val2)
{
	...
	case 3:
		...
		break;
	case 8:
		config_ipq50xx_LANWANPartition(val2);
		break;
	case 13:
		get_ipq50xx_WAN_Speed(&value2);
		printf("WAN speed : %u Mbps\n", value2);
		break;
	case 14: // Link up LAN ports
		link_down_up_ipq50xx_PHY(get_lan_port_mask(), 1);
		break;
	case 15: // Link down LAN ports
		link_down_up_ipq50xx_PHY(get_lan_port_mask(), 0);
		break;
	case 16: // Link up ALL ports
		link_down_up_ipq50xx_PHY(wanlanports_mask, 1);
		break;
	case 17: // Link down ALL ports
		link_down_up_ipq50xx_PHY(wanlanports_mask, 0);
		break;
	case 27:
		reset_qca_switch();
		break;
	...
	case 114: // link up WAN ports
		for (i = WAN_UNIT_FIRST; i <= max_wan_unit; ++i)
			link_down_up_ipq50xx_PHY(get_wan_port_mask(i), 1);
		break;
	case 115: // link down WAN ports
		for (i = WAN_UNIT_FIRST; i <= max_wan_unit; ++i)
			link_down_up_ipq50xx_PHY(get_wan_port_mask(i), 0);
		break;
...
rtkswitch_Port_phyStatus(unsigned int port_mask)
{
	...
	get_ipq50xx_phy_linkStatus(port_mask, &status);
...
rtkswitch_Port_phyLinkRate(unsigned int port_mask)
{
	...
	get_ipq50xx_Port_Speed(port_mask, &speed);
...
rtkswitch_wanPort_phyStatus(int wan_unit)
{
	...
	get_ipq50xx_phy_linkStatus(get_wan_port_mask(wan_unit), &status);
...
rtkswitch_lanPorts_phyStatus(void)
{
	...
	get_ipq50xx_phy_linkStatus(get_lan_port_mask(), &status);
...
void ATE_port_status(int verbose, phy_info_list *list)
{
	...
	for (i = 0; i < NR_WANLAN_PORT; i++) {
		...
		if (vport_to_phy_addr[i] >= 0)
			get_ipq50xx_vport_info(i, &pS.link[i], &pS.speed[i]);
	}
