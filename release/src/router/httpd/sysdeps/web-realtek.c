...
"<head>"
"<title>Realtek Home Gateway Reference Design: Apply</title>"
...
"</form>"
"<p class=\"label\">&#169;2001-2004 Realtek Corporation. All rights reserved.</p>"
"</body>"
;
#endif
...
static int
dump_rtk_bss_info(int band,int eid, webs_t wp, int argc, char_t **argv, bss_info *bss)
{
...
//cprintf("%s:%d\n",__FUNCTION__,__LINE__);
	...
	switch(bss->state) 
		{
		case STATE_DISABLED:
			strlcpy(statebuf,"Disabled", sizeof(statebuf));
			break;
		case STATE_IDLE:
			strlcpy(statebuf,"Idle", sizeof(statebuf));
			break;
		case STATE_STARTED:
			strlcpy(statebuf,"Started", sizeof(statebuf));
			break;
		case STATE_CONNECTED:
			strlcpy(statebuf,"Connected", sizeof(statebuf));
			break;
		case STATE_WAITFORKEY:
			strlcpy(statebuf,"Waiting for keys", sizeof(statebuf));
			break;
		case STATE_SCANNING:
			strlcpy(statebuf,"Scanning", sizeof(statebuf));
			break;
		default:
			strlcpy(statebuf,"unknown", sizeof(statebuf));
		}
	retval += websWrite(wp, "STATE: \"%s\"\t", statebuf);
	retval += websWrite(wp, "Channel: \"%d\"\t", bss->channel);

	//retval += websWrite(wp, "RSSI: %d dBm\t", bss->rssi);

	//retval += websWrite(wp, "SQ: %d \n", bss->sq);
	retval += websWrite(wp, "BSSID: %s\n", wl_ether_etoa((struct ether_addr*)&(bss->bssid)));	
	...
	if ((ret = wl_ioctl(name, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN)) == 0) {
		...
		*(uint32*)buf = htod32(WLC_IOCTL_MAXLEN);
		if ((ret = wl_ioctl(name, WLC_GET_BSS_INFO, buf, WLC_IOCTL_MAXLEN)) < 0)
			return 0;
#ifdef RTCONFIG_REALTEK
		rtk_bss=(bss_info*)buf;
		retval += dump_rtk_bss_info(unit, eid, wp, argc, argv, rtk_bss);
#else
...
//		ret += websWrite(wp, "Mode	: Ethernet Bridge\n");
		...
		if (nvram_get_int("wlc_express") == (unit+1)) { // Express Way
			if (unit == 0)
				ret += websWrite(wp, "Mode	:  Express Way 2 GHz\n");
			else
				ret += websWrite(wp, "Mode	:  Express Way 5 GHz\n");
			goto exit;
		}
		else
#endif
...
//		return ret;
	...
	strlcpy((char*)staInfoList,"rtk_sta_info", MAX_STA_NUM+1);
	if (wl_ioctl(name, WLC_GET_VAR, staInfoList, sizeof(staInfoList)))
			goto exit;
	...
	ret += websWrite(wp, "%-18s%-8s%-12s%-12s%-15s%-15s\n",
				"MAC", "Mode", "  Tx Packet", "  Rx Packet",  "  Power Saving", "  Connect Time");

	for(i=1; i<=MAX_STA_NUM; i++)
	{
		//cprintf("%s:%d i=%d aid=%d\n",__FUNCTION__,__LINE__,i,staInfoList[i].aid);
		if (staInfoList[i].aid && (staInfoList[i].flag & STA_INFO_FLAG_ASOC))
		{
			if(staInfoList[i].network& BAND_5G_11AC)
				snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11ac"));
			else if(staInfoList[i].network & BAND_11N)
				snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11n"));
			else if (staInfoList[i].network & BAND_11G)
				snprintf(mode_buf, sizeof(mode_buf),"%s",  (" 11g"));
			else if (staInfoList[i].network & BAND_11B)
				snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11b"));
			else if (staInfoList[i].network& BAND_11A)
				snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11a"));
			else
			...
			ret+=websWrite(wp,"%s",wl_ether_etoa((struct ether_addr*)&(staInfoList[i].addr)));
	
			ret += websWrite(wp, "%-8s%12lu%12lu%15s   %02d:%02d:%02d\n",
	...
	for (i = 1; i < 4; i++) {
		...
		if (nvram_match(strcat_r(prefix, "bss_enabled", tmp), "1"))
		{
			...
			strlcpy((char*)staInfoList,"rtk_sta_info", MAX_STA_NUM+1);
			if (wl_ioctl(name_vif, WLC_GET_VAR, staInfoList, sizeof(staInfoList)))
					goto exit;

			for(ii=1; ii<=MAX_STA_NUM; ii++)
			{
				cprintf("%s:%d ii=%d aid=%d\n",__FUNCTION__,__LINE__,i,staInfoList[ii].aid);
				if (staInfoList[ii].aid && (staInfoList[ii].flag & STA_INFO_FLAG_ASOC))
				{
					if(staInfoList[ii].network& BAND_5G_11AC)
						snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11ac"));
					else if(staInfoList[ii].network & BAND_11N)
						snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11n"));
					else if (staInfoList[ii].network & BAND_11G)
						snprintf(mode_buf, sizeof(mode_buf),"%s",  (" 11g"));
					else if (staInfoList[ii].network & BAND_11B)
						snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11b"));
					else if (staInfoList[ii].network& BAND_11A)
						snprintf(mode_buf, sizeof(mode_buf), "%s", (" 11a"));
					else
						snprintf(mode_buf, sizeof(mode_buf), "%s", (" ---"));
					hr = staInfoList[ii].link_time / 3600;
					min = (staInfoList[ii].link_time % 3600) / 60;
					sec = staInfoList[ii].link_time - hr * 3600 - min * 60;

					ret += websWrite(wp, "%02x:%02x:%02x:%02x:%02x:%02x%-8s%-12lu%-12lu%-15s   %02d:%02d:%02d\n", staInfoList[ii].addr[0],staInfoList[ii].addr[1],staInfoList[ii].addr[2],staInfoList[ii].addr[3],staInfoList[ii].addr[4],staInfoList[ii].addr[5],\
						mode_buf,staInfoList[ii].tx_packets, staInfoList[ii].rx_packets,\
						((staInfoList[ii].flag & STA_INFO_FLAG_ASLEEP) ? "yes" : "no"),
						hr, min, sec);
...
static int ej_wl_channel_list(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if (is_wlif_up(name) != 1)
	{
		...
		foreach (word, nvram_safe_get(strcat_r(prefix, "chlist", tmp)), next) {
			...
			{
				snprintf(tmp1_t, sizeof(tmp1_t),  "%s \"%s\"]", tmp1, word);
	...
	for (i = 0; i < dtoh32(list->count) && i < IW_MAX_FREQUENCIES; i++) {
		...
		{
			snprintf(tmp1_t, sizeof(tmp1_t),  "%s \"%d\"]", tmp1, ch);
			strlcpy(tmp1, tmp1_t, sizeof(tmp1));
			snprintf(tmp2_t, sizeof(tmp2_t),  "%s %d", tmp2, ch);
...
static int ej_wl_rate(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	else
	{
		...
		if ((rate == -1) || (rate == 0))
			...
			if(rate % 10)
				snprintf(rate_buf, sizeof(rate_buf), "%d.%d Mbps", (rate / 10), (rate % 10));
			else
				snprintf(rate_buf, sizeof(rate_buf), "%d Mbps", (rate / 10));
		}
...
ERROR:
	if(from_app == 0)
		...
		retval += websWrite(wp, "\"%s\"", rate_buf);
	//cprintf("%s:%d rate_buf=%s\n",__FUNCTION__,__LINE__,rate_buf);
	return retval;
}
...
getWscStatusStr()
{
	...
	struct stat f_stat;
	int status = -1;
	static int keep_status = -1;
	static int keep_time = 0;

	if (!stat(WSCD_BEFORE_STOP_STATUS, &f_stat)) {// file exist
		fp = fopen(WSCD_BEFORE_STOP_STATUS, "r");
	}
	else
	...
	if (fp) {
		...
		if (!stat(WSCD_BEFORE_STOP_STATUS, &f_stat)) { // file exist
			if (status <= PROTOCOL_START || (status >= SEND_EAPOL_START && status <= PROC_EAP_ACK)) {
				// skip processing/idle status and get new status from WSCD_CONFIG_STATUS
				fp = fopen(WSCD_CONFIG_STATUS, "r");
				if (fp == NULL) {
			...
			unlink(WSCD_BEFORE_STOP_STATUS);
		}

		/* If status is Success or Error, keep the msg in 60 secs. */
		if (status == PROTOCOL_PBC_OVERLAPPING || status == PROTOCOL_TIMEOUT || status == PROTOCOL_SUCCESS ||
		     status ==  WSC_EAP_FAIL || status == HASH_FAIL || status == HMAC_FAIL || status == PWD_AUTH_FAIL ||
		     status == PROTOCOL_PIN_NUM_ERR || status == PROTOCOL_ERR || status == PROC_EAP_DONE || status == PROTOCOL_ERR) {
			keep_status = status;
			keep_time = 60;
		}
		else {
			if (keep_time && status == NOT_USED) { // Just skip NOT_USED
				status = keep_status;
				keep_time--;
			}
			else {
				keep_status = NOT_USED;
				keep_time = 0;
			}
		}
		...
		if (nat_list) {
			...
			if (netconf_get_nat(nat_list, &listlen) == 0 && needlen == listlen) {
				...
				if (nat_list[i].target==NETCONF_DNAT)
				{
					...
					{
						snprintf(line, sizeof(line), "%-15s", "all");
					}
					else
					{
						snprintf(line, sizeof(line), "%-15s", inet_ntoa(nat_list[i].match.dst.ipaddr));
					}

					if (ntohs(nat_list[i].match.dst.ports[0])==0){
						snprintf(line_t, sizeof(line_t), "%s %-7s", line, "ALL");
						strlcpy(line, line_t, sizeof(line));
					}
					else if (nat_list[i].match.ipproto==IPPROTO_TCP){
						snprintf(line_t, sizeof(line_t), "%s %-7s", line, "TCP");
						strlcpy(line, line_t, sizeof(line));
					}
					else{
						snprintf(line_t, sizeof(line_t), "%s %-7s", line, "UDP");
						strlcpy(line, line_t, sizeof(line));
					}
					...
					{
						if (ntohs(nat_list[i].match.dst.ports[0])==0){
							snprintf(line_t, sizeof(line_t), "%s %-11s", line, "ALL");
							strlcpy(line, line_t, sizeof(line));
						}
						else{
							snprintf(line_t, sizeof(line_t), "%s %-11d", line, ntohs(nat_list[i].match.dst.ports[0]));
							strlcpy(line, line_t, sizeof(line));
						}
					}
					else 
					{
						snprintf(tstr, sizeof(tstr), "%d:%d", ntohs(nat_list[i].match.dst.ports[0]),
						ntohs(nat_list[i].match.dst.ports[1]));
						snprintf(line_t, sizeof(line_t), "%s %-11s", line, tstr);
						strlcpy(line, line_t, sizeof(line));
					}
					snprintf(line_t, sizeof(line_t), "%s %s\n", line, inet_ntoa(nat_list[i].ipaddr));
					strlcpy(line, line_t, sizeof(line));
...
//	params->scan_type = DOT11_SCANTYPE_PASSIVE;
	...
	if (!ret)
	{
		...
		{
			snprintf(ure_mac, sizeof(ure_mac), "%02X:%02X:%02X:%02X:%02X:%02X", (unsigned char)bssid[0],
	...
	if (strstr(nvram_safe_get("wl0_akm"), "psk"))
	{
		...
		strlcpy((char*)authorized, "autho_sta_list", maclist_size);
...
static int wl_sta_list(int eid, webs_t wp, int argc, char_t **argv, int unit) {
	...
	/* query wl for authenticated sta list */
	strlcpy((char*)auth, "authe_sta_list", maclist_size);
...
int ej_wl_auth_list(int eid, webs_t wp, int argc, char_t **argv) {
	...
	foreach (word, nvram_safe_get("wl_ifnames"), next) {
		...
		/*strlcpy((char*)wme, "wme_sta_list", maclist_size);
...
wl_get_scan_results(char *ifname)
{
	...
	while ((ret = wl_ioctl(ifname, WLC_SCAN, apinfos, sizeof(apinfos))) < 0 && count++ < 2)
	{
		fprintf(stderr, "set scan command failed, retry %d\n", count);
		sleep(1);
	}
	...
	{
		count = 0;
		while ((ret = wl_ioctl(ifname, WLC_SCAN_RESULTS, apinfos, sizeof(apinfos))) < 0 && count++ < 2)
		{
			fprintf(stderr, "set scan results command failed, retry %d\n", count);
			sleep(1);
		}
...
wl_scan(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if (apinfo_count)
	{
		...
		{
			memset(ssid_str, 0, sizeof(ssid_str));
			char_to_ascii(ssid_str, trim_r((char *)apinfos[i].SSID));

			/*bssidp = (unsigned char *)&apinfos[i].BSSID;
			...
			strlcpy(macstr,apinfos[i].BSSID, sizeof(macstr));
