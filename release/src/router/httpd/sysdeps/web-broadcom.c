...
wl_status(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	snprintf(prefix2, sizeof(prefix2), "wl%d.%d_", unit, 1);

	if ((repeater_mode() || psr_mode()) &&
		(unit == nvram_get_int("wlc_band")) &&
		nvram_match(strlcat_r(prefix2, "bss_enabled", tmp, sizeof(tmp)), "1"))
		strlcpy(ifname, nvram_safe_get(strlcat_r(prefix2, "ifname", tmp, sizeof(tmp))), sizeof(ifname));
	else
...
#ifdef RTCONFIG_WIFI6E
			(nvram_match(strlcat_r(prefix, "nband", tmp, sizeof(tmp)), "4") ? "6 GHz" :
...
#endif
	if ((nvram_match(strlcat_r(prefix, "mode", tmp, sizeof(tmp)), "ap")
	  || nvram_match(strlcat_r(prefix, "mode", tmp, sizeof(tmp)), "wds"))
		&& !nvram_match(strlcat_r(prefix, "wds", tmp, sizeof(tmp)), "")) {
		ret += websWrite(wp, "\n");
		ret += websWrite(wp, "Bridge List                             \n");
		ret += websWrite(wp, "----------------------------------------\n");
		ret += websWrite(wp, "%-4s%-18s%-7s\n", "idx", "MAC", "Status");

		strlcpy(macs, nvram_safe_get(strlcat_r(prefix, "ifname", tmp, sizeof(tmp))), sizeof(macs));
		i = 1;
		foreach(mac, macs, next) {
			...
			if (atoi(nvram_safe_get(strlcat_r(prefix, "wds_timeout", tmp, sizeof(tmp)))) &&
...
ej_wl_status_2g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for (ii = 0; ii < DEV_NUMIFS; ii++) {
#ifdef GTAXE16000
		snprintf(nv_param, sizeof(nv_param), "wl%d_unit", wl_order[ii]);
#else
		...
		{
			retval += ej_wl_status(eid, wp, argc, argv, atoi(temp));
			retval += websWrite(wp, "\n");
		}
	}
...
ej_wl_control_channel(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	foreach (word, wl_ifnames, next){
		if(count_wl_if != 0)
			ret += websWrite(wp, ", ");
		ret += websWrite(wp, "\"%d\"", wl_control_channel(count_wl_if));
		count_wl_if++;
	}
	ret += websWrite(wp, "]");
...
static int ej_wl_channel_list(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	snprintf(prefix, sizeof(prefix), "wl%d_", unit);
	strlcpy(ifname, nvram_safe_get(strlcat_r(prefix, "ifname", tmp, sizeof(tmp))), sizeof(ifname));
	strlcpy(chlist, nvram_safe_get(strlcat_r(prefix, "chlist", tmp, sizeof(tmp))), sizeof(chlist));
...
ej_wl_chanspecs(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	snprintf(prefix, sizeof(prefix), "wl%d_", unit);
	strlcpy(ifname, nvram_safe_get(strlcat_r(prefix, "ifname", tmp, sizeof(tmp))), sizeof(ifname));
	strlcpy(chansps, nvram_safe_get(strlcat_r(prefix, "chansps", tmp, sizeof(tmp))), sizeof(chansps));
	...
	for (i = 0; i < count; i++) {
...
#ifdef RTAX3000N
		if ((get_model() == MODEL_RTAX3000N) &&
			!strcmp(get_productid(), "RT-AX55_V2") &&
			strstr(chanbuf, "/160")) {
			snprintf(tmpx, sizeof(tmpx), "%s \"%s\"]", tmp1, "0");
			strlcpy(tmp1, tmpx, sizeof(tmp1));
			break;
		}
#endif
		...
		if (strlen(tmp2))
			nvram_set(strlcat_r(prefix, "chansps", tmp, sizeof(tmp)), tmp2);
	}
...
static int ej_wl_rssi(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	snprintf(prefix, sizeof(prefix), "wl%d_", unit);
	strlcpy(ifname, nvram_safe_get(strlcat_r(prefix, "ifname", tmp, sizeof(tmp))), sizeof(ifname));
	mode = nvram_safe_get(strlcat_r(prefix, "mode", tmp, sizeof(tmp)));
	sta = !strcmp(mode, "sta");
	wet = !strcmp(mode, "wet");
...
static int ej_wl_rate(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if (nvram_match(strlcat_r(prefix, "mode", tmp, sizeof(tmp)), "wet")) {
...
ej_wl_rate_2g(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	for(i=0; i<(sizeof(wl_band_list)/sizeof(wl_band_list[0])) && wl_band_list[i][0] != '\0'; i++){
		if(strncmp("2g1", wl_band_list[i], 3) == 0)
			break;
	}
	return ej_wl_rate(eid, wp, argc, argv, i);
}
...
ej_wl_rate_5g(int eid, webs_t wp, int argc, char_t **argv)
{
	int i = 0;
	for(i=0; i<(sizeof(wl_band_list)/sizeof(wl_band_list[0])) && wl_band_list[i][0] != '\0'; i++){
		if(strncmp("5g1", wl_band_list[i], 3) == 0)
			break;
	}
	return ej_wl_rate(eid, wp, argc, argv, i);
}
...
ej_wl_rate_5g_2(int eid, webs_t wp, int argc, char_t **argv)
{
	int i = 0;
	for(i=0; i<(sizeof(wl_band_list)/sizeof(wl_band_list[0])) && wl_band_list[i][0] != '\0'; i++){
		if(strncmp("5g2", wl_band_list[i], 3) == 0)
			break;
	}
	return ej_wl_rate(eid, wp, argc, argv, i);
}
...
ej_wl_rate_6g(int eid, webs_t wp, int argc, char_t **argv)
{
	int i = 0;
	for(i=0; i<(sizeof(wl_band_list)/sizeof(wl_band_list[0])) && wl_band_list[i][0] != '\0'; i++){
		if(strncmp("6g1", wl_band_list[i], 3) == 0)
			break;
	}
	return ej_wl_rate(eid, wp, argc, argv, i);
}
...
{
	return ej_wl_cap(eid, wp, argc, argv, 3);
}
...
int
ej_wl_chipnum_6g(int eid, webs_t wp, int argc, char **argv)
{
	return ej_wl_chipnum(eid, wp, argc, argv, 3);
}
...
//					strlcpy(apinfos[ap_count].SSID, info->SSID, sizeof(apinfos[ap_count].SSID));
...
next_info:
	...
	if (strstr(nvram_safe_get("wl0_akm"), "psk"))
	{
		...
		strlcpy((char*)authorized, "autho_sta_list", mac_list_size);
	...
	if (nvram_match(strlcat_r(prefix, "reg_mode", tmp, sizeof(tmp)), "h") && !is_psta(unit)) {
		...
		if (((ctl_ch > 48) && (ctl_ch < 149))
			...
			else
			{
				...
				chspec_tmp = (((nvram_get_hex(strlcat_r(prefix, "band5grp", tmp, sizeof(tmp))) & WL_5G_BAND_4) && (ctl_ch < 100)) ? select_chspec_with_band_bw(ifname, 4, 3, chspec_cur) : select_chspec_with_band_bw(ifname, 1, 3, chspec_cur));
...
#ifdef __CONFIG_DHDAP__
	if (is_dhd && !nvram_match(strlcat_r(prefix, "mode", tmp, sizeof(tmp)), "wds")) {
...
ej_wl_auth_psta(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (wl_ioctl(ifname, WLC_GET_SSID, &ssid, sizeof(ssid)))
		goto PSTA_ERR;
	else if (strncmp(nvram_safe_get(strlcat_r(prefix, "ssid", tmp, sizeof(tmp))), (const char *) ssid.SSID, strlen(nvram_safe_get(strlcat_r(prefix, "ssid", tmp, sizeof(tmp))))))
		goto PSTA_ERR;
